...
var Enumerable = (function() {
  function ensureEach(obj) {
    // sajana - 2013-05-07
    // nasty hack to ensure that each will work.
    // A bug between Google's ad library and Prototype
    // caused problems due to Prototype implementing Array.some
    // and Google thinking it would only be implemented if the
    // browser natively implemented it.  Google cached this method
    // and then used it on an Array where .each was not implemented
    // (the Array was created in an iframe). So we check to see
    // if each, _each, and forEach are implemented.
    obj = obj || this;
    if (!obj.each) {
      obj.each = each;
      if (!obj._each) {
        // try to use native methods if possible
        if ([].forEach) {
          obj._each = [].forEach;
        } else {
          obj._each = function(f) {
            var x = 0;
  ...
  return {
    ...
    every:      all,
    any:        any,
    // use native methods when we can
    some:       [].some || any,
    collect:    collect,
    map:        collect,
    detect:     detect,
